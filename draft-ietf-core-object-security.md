...
For each endpoint, the security context is composed by a "Common Context", a "Sender Context" and a "Recipient Context". The Common Context includes common parameters. Each endpoint has a unique ID called Sender ID used to derive its Sender Context. Endpoints protect messages sent using the Sender Context and verifies messages received using the Recipient Context. The Recipient Context is derived with the other endpoint's ID, called Recipient ID. In communication between two endpoints, the Sender Context of one endpoint matches the Recipient Context of the other endpoint, and vice versa. Note that, because of that, the two security contexts identified by the same Context Identifiers in the two endpoints are not the same, but they are partly mirrored. Retrieval and use the security contexts the Security Context are shown in {{sec-context-ex}}.
...
* Sender ID
   - Defaults are 0x00 for Client and 0x01 for Sever
* Recipient ID
   - Defaults are 0x01 for Client and 0x00 for Sever
...
Given the input parameters, the client and server can derive all the other parameters in the security context. The same base_key SHOULD NOT be used with more than one Cid, and MUST NOT be used several times with the same Cid.
...
As mentioned, Cid, are established in a previous phase. How this is done is application specific, but it is RECOMMENDED that the application uses 64-bits long pseudo-random Cids, in order to have globally unique Context Identifiers. Cid SHOULD be unique in the sets of all security contexts used by all the endpoints. If it is not the case, it is the role of the application to specify how to handle collisions.

If the application is total control of both clients and servers, shorter unique Cids MAY be used. Note that Cids of different lengths can be used by	different clients and that e.g. a Cid with the value 0x00 is	different from the Cid with the value 0x0000.
...
In applications where the Client and Server roles are not changing, the default values can be used.
